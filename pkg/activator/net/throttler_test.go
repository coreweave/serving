...
func TestThrottlerErrorNoRevision(t *testing.T) {
	...
	if err := throttler.Try(ctx, revID, func(string) error { return innerError }); !errors.Is(
		err,
		innerError,
	) {
	...
	servfake.ServingV1().
		Revisions(revision.Namespace).
		Delete(ctx, revision.Name, metav1.DeleteOptions{})
	...
	if lastError == nil ||
		lastError.Error() != `revision.serving.knative.dev "test-revision" not found` {
...
func TestThrottlerSuccesses(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			servfake.ServingV1().
				Revisions(tc.revision.Namespace).
				Create(ctx, tc.revision, metav1.CreateOptions{})
...
func TestPodAssignmentFinite(t *testing.T) {
	...
	if got, want := trackerDestSet(rt.assignedTrackers), sets.NewString("ip0", "ip4"); !got.Equal(
		want,
	) {
...
func TestActivatorsIndexUpdate(t *testing.T) {
	...
	waitInformers, err := rtesting.RunAndSyncInformers(
		ctx,
		endpoints.Informer(),
		revisions.Informer(),
	)
...
func TestMultipleActivators(t *testing.T) {
	...
	if result := <-resultChan; !possibleDests.Has(result.dest) {
		t.Fatalf(
			"Request went to an unknown destination: %s, possibles: %v",
			result.dest,
			possibleDests,
		)
	}
